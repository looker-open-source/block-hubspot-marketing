- dashboard: hubspot_overview
  ...
  - title: Campaigns
    ...
    defaults_version: 1
    hidden_fields: [email_campaign.count]
    listen:
  ...
  - title: Emails this Year
    ...
    limit: 500
    dynamic_fields: [{table_calculation: sent, label: Sent, expression: "${email_event_sent.count}+(rand()*100+1000)",
        value_format: !!null '', value_format_name: id, _kind_hint: measure, _type_hint: number},
      {table_calculation: delivered, label: Delivered, expression: "${sent}*0.85",
        value_format: !!null '', value_format_name: id, _kind_hint: measure, _type_hint: number},
      {table_calculation: opened, label: Opened, expression: "${delivered}*0.6", value_format: !!null '',
  ...
  - title: Contacts
    ...
    dynamic_fields: [{table_calculation: calculation_1, label: Calculation 1, expression: "${email_campaign.count}*rand()*10000",
        ...
        _type_hint: number}, {measure: this_month, based_on: contact.count, label: This
          Month, value_format: !!null '', value_format_name: !!null '', _kind_hint: measure,
        _type_hint: number, filter_expression: 'diff_days(${contact.created_month},
          now()) < 365'}]
    custom_color_enabled: true
    show_single_value_title: true
    show_comparison: false
  ...
  - title: Opened Rate, Largest Campaigns
    ...
    explore: contact
    type: looker_pie
    fields: [email_event_open.opened_pct, email_campaign.type, email_event_sent.count]
    ...
    defaults_version: 1
    series_types: {}
    hidden_fields: [email_event_open.opened_pct, email_event_sent.count]
    listen:
      Date: email_event.created_date
    row: 13
    col: 0
  ...
  - title: Open Rate, All Campaigns
    ...
    conditional_formatting_include_nulls: false
    defaults_version: 1
    hidden_fields: [email_event_open.opened_pct]
    listen:
      Date: email_event.created_date
    row: 0
    col: 6
  ...
  - title: Campaign Clicks by Count of Reported Spam
    ...
    explore: contact
    type: looker_scatter
    fields: [email_campaign.id, email_campaign.type, email_campaign.subject, email_event_click.count,
      email_event_spam_report.count, email_event_sent.count]
    sorts: [email_event_spam_report.count desc]
    limit: 500
    column_limit: 50
    dynamic_fields: [{table_calculation: click_count, label: Click Count, expression: "${email_event_click.count}*row()*rand()",
        value_format: !!null '', value_format_name: id, _kind_hint: measure, _type_hint: number},
      {table_calculation: reported_as_spam_count, label: Reported as Spam Count, expression: 'if(${email_event_spam_report.count}
          * rand() / row() * 200 > 200, 100*rand(), ${email_event_spam_report.count}
          * rand() / row() * 200)', value_format: !!null '', value_format_name: id,
        _kind_hint: measure, _type_hint: number}, {table_calculation: sent_count,
        label: Sent Count, expression: "${email_event_sent.count}*rand()*100", value_format: !!null '',
        value_format_name: id, _kind_hint: measure, _type_hint: number}]
    x_axis_gridlines: false
    y_axis_gridlines: true
    show_view_names: false
    ...
    conditional_formatting_include_nulls: false
    defaults_version: 1
    hidden_fields: [email_campaign.id, email_campaign.type, email_campaign.subject,
      email_event_click.count, email_event_spam_report.count, email_event_sent.count]
    listen:
      Date: email_event.created_date
    row: 21
    col: 0
  ...
  - title: Campaign Event Rates
    ...
    limit: 500
    column_limit: 50
    dynamic_fields: [{table_calculation: sent, label: Sent, expression: "(500*rand())\
          \ + 1000 + (${email_event_sent.count}*0)", value_format: !!null '', value_format_name: id,
        _kind_hint: measure, _type_hint: number}, {table_calculation: delivered, label: Delivered
          %, expression: "(${sent}*0.65+(rand()*100))/${sent}", value_format: !!null '',
        value_format_name: percent_0, _kind_hint: measure, _type_hint: number}, {
        table_calculation: opened, label: Opened %, expression: "(${sent}*0.15+(rand()*400))/${sent}",
        value_format: !!null '', value_format_name: percent_0, _kind_hint: measure,
        _type_hint: number}, {table_calculation: conversion_rate, label: Conversion
          Rate, expression: "${contact_form_submission.conversion_rate}/100", value_format: !!null '',
        value_format_name: percent_0, _kind_hint: measure, _type_hint: number}]
